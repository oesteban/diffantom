...
\Gls*{csf} was simulated with a unique compartment with isotropic diffusivity $D_{CSF}$ of 3.0$\times$10$^{-6}$ [mm s$^{-1}$].
Conversely, \Gls*{wm} and \gls*{gm} included one isotropic compartment with $D_{WM} =$ and $D_{GM} =$, respectively.
Single fiber contributions had $D_{\parallel}=$ and $D_{\bot}=$.
All these values for diffusivity can be modified by the user for custom settings.
