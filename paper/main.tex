...
\title[Diffantom]{Diffantom: a whole-brain diffusion MRI phantom derived from a real dataset of the \acrlong{hcp}}
...
We also openly release \emph{MkDiffantom}, the software package necessary to generate custom \emph{diffantoms}.
...
On one hand, \emph{MkDiffantom} requires up to five fraction maps \tmaps{} of
...
The process to obtain the microstructural model from one dataset of the \gls*{hcp} can be described
  as follows (see also \autoref{fig:figure1}, box B):
1) The fiber orientation maps $\{\mathbf{V}_i\}$ and their corresponding estimations of volume fraction $\{F'_i\}$ are
  obtained using the ball-and-stick model for multi-shell data of \emph{bedpostx} \citep{jbabdi_modelbased_2012}
  on the \gls*{dmri} data.
The \gls*{hcp} recommends \emph{bedpostx} to reconstruct their data \citep{glasser_minimal_2013}.
A further advantage is that \emph{bedpostx} exploits the multi-shell acquisitions of the \gls*{hcp} while
  operating at whole-brain level.
2) A \gls*{fa} map is obtained after fitting a tensor model with \emph{MRTrix}.
The \gls*{fa} is used to infer $F_1$ (the fraction map of the most prevalent fiber), avoiding the extremely
  noisy estimation of $F'_1$ performed by \emph{bedpostx} in the previous step.
...
We also provide \emph{MkDiffantom}, the software package used to generate \emph{diffantoms}, so
...
\autoref{fig:figure2}, box A1, shows the result of the tractography obtained with such pipeline.
Finally, we applied \emph{tract querier} \citep{wassermann_on_2013} on segmenting important fiber bundles such
  as the \gls*{cst} and the forceps minor (see \autoref{fig:figure2}, box A2).
...
We recommend \emph{Diffantom} as ground-truth in verification and validation frameworks
  (\autoref{fig:figure2}, box B) for testing pipelines.
...
\begin{itemize}
  ...
  \item Using \emph{Diffantom} as in box B of \autoref{fig:figure2}, if is possible to apply binary classification measures
...
The software workflow released with this data report (\emph{MkDiffantom}) ensures the reproducibility of
  \emph{Diffantom} and enables the generation of custom \emph{diffantoms}.
\emph{MkDiffantom} is designed for, but not limited to, use \gls*{hcp} datasets as source of structural information.
...
\emph{Diffantom} is presented here to be openly and freely distributed along with \emph{MkDiffantom}
...
FA denotes the \gls*{fa} map obtained from the original \gls*{dmri} data: the local fiber orientation maps
  $\{\mathbf{V}_i\}$ with their estimated volume fractions $\{F'_i\}$ calculated with \emph{bedpostx}.
...
\textbf{B. \emph{MkDiffantom}, a workflow to generate \emph{diffantoms}}.
...
\textbf{A1. Tractogram of fibers crossing slice 56 of \emph{Diffantom}} as extracted with \emph{MRTrix},
  represented over that corresponding slice of the \emph{b0} volume.
\textbf{A2. Segmentation of some fiber bundles using \emph{tract\_querier}}: the right \gls*{cst} is
