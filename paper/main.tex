...
\title[Diffantom]{Diffantom: a whole-brain diffusion MRI phantoms derived from real datasets of the \acrlong{hcp}}
...
Once a microstructural model of the volume has been synthesized, a map of \glspl{fodf} is computed using spherical
...
A close-up showing how the \glspl{fodf} map looks is presented in \autoref{fig:figure1}-B.
...
The resulting \glspl{fodf} map is then combined with the free- and hindered-diffusion compartments corresponding to $\{T_j\}$.
...
\begin{itemize}
  ...
  \item Using \emph{Diffantom} as in box B of \autoref{fig:figure2}, it is possible to apply binary classification measures
...
\textbf{A1. Example dataset.} The tractogram of fibers crossing slice 56 of \emph{Diffantom} as
  extracted with \emph{MRTrix} is presented, represented over the corresponding slice of the
  \lowb{} volume.
\textbf{A2. Segmentation of some fiber bundles in the example dataset.} We used \emph{tract\_querier} to
  segment the right \gls*{cst} represented with blue streamlines, the left \gls*{cst} (red streamlines),
  and the forceps minor (green streamlines).
  The box includes the slice 56 of the \lowb{} and the pial surface is represented with transparency.
