...
\def\Address{%
$^{1}$Biomedical Image Technologies (BIT), ETSI Telecomunicaci\'on, Universidad Polit\'ecnica de Madrid, Madrid, Spain \\
...
\emph{Diffantom} observes two major improvements with respect to \citep{wilkins_fiber_2014}.
First, since we use a dataset from the \gls*{hcp} as input, data are corrected for the most relevant distortions.
\cite{wilkins_fiber_2014} explicitly state that their original data did not subjected to certain distortion
  corrections, and thus, generated data are affected correspondingly.
Second improvement is methodological, as we use a more complete signal model to generate the phantom thanks
  to \emph{phantomas} \citep{caruyer_phantomas_2014}.
...
In order to release a reproducible dataset, along with an usable software to create custom data,
  we also distribute the software tools summarized in \autoref{fig:figure1}.

\section{Discussion and conclusion}

\noindent\textbf{\textit{Discussion\textcolon}}\label{sec:discussion} %
Whole-brain, realistic \gls*{dmri} phantoms are necessary in current development of connectomics.
The \emph{diffantom} is a derivative work from \citep{wilkins_fiber_2014} in terms of methodology for
...
\end{center}
\textbf{\refstepcounter{figure}\label{fig:figure1} Figure \arabic{figure}.}{ Enter the caption for your figure here.  Repeat as  necessary for each of your figures }
\end{figure}
