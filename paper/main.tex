...
These analyses have been successfully applied in the definition of the unique subject-wise patterns of connectivity
  ...
  study of the link of structural and functional connectivity \citep{messe_predicting_2015}.
...
In this report we present \emph{diffantom}, an \emph{in-silico} dataset to assess tractography pipelines using
  real data as source of whole-brain micro-structural information.
We also provide a full software layer to produce \emph{diffantoms} from real \gls*{dmri} datasets
  drawn from the \gls*{hcp}.
This dataset enables the following evaluations of standard connectivity pipelines:
\begin{itemize}
  \item Comparisons of the impact of the different diffusion sampling schemes on the local micro-structure
  model of choice and the subsequent global tractography outcome.
  \item Investigations on the effects of the different artifacts (noise, \glspl*{pve} and \gls*{csf} contamination,
    susceptibility-derived warping, Eddy-currents-derived distortions, etc.) at both local and global levels.
  \item Determine of the \acrfull*{roc} of connectivity pipelines.
  \item Generation of synthetic connectivity networks to test graph theory analyses.
  \item Simulation of pathological brains (i.e. tumors as in \cite{kaus_simulation_2000}).
\end{itemize}
...
In order to generate \emph{a diffantom}, the simulation inputs (see \autoref{fig:figure1}, A) are computed.
On one hand, the software requires up to five volume fractions $\{T_{1\,\cdots\,5}\}$ corresponding to the isotropic
  compartments that will be simulated.
The isotropic fraction maps use the \emph{5TT format} established with \emph{MRTrix3} \citep{tournier_mrtrix_2012}
  that describes the structure of the brain in the following order: \gls*{cgm}, \gls*{dgm}, \gls*{wm}, \gls*{csf},
  and abnormal tissue.
Since we simulated a healthy subject, the last fraction map is omitted.
On the other hand, the anisotropic compartments are specified by up to three volume fractions $\{F_{1\,\cdots\,3}\}$
  of three single fiber populations per voxel along with their corresponding direction maps $\{V_{1\,\cdots\,3}\}$.
The process to obtain these inputs is described as follows:
1) The fiber orientation maps $\{V_{1\,\cdots\,3}\}$ and their corresponding single-fiber fraction maps are
...
2) We obtain a \gls*{fa} map after fitting a tensor model with \emph{MRTrix3}.
3) The original single-fiber fractions and the \gls*{fa} map are smoothed with a non-local means filter included
...
5) The objects obtained in the previous steps are combined as described in the \nameref{sec:appendix} to generate the
  outcome presented in \autoref{fig:figure1}, box A.
...
Once a micro-structural model of the volume has been synthesized, a map of \glspl{fod} is computed using spherical
  ...
  corresponding single-fiber fractions $\{F_{1\,\cdots\,3}\}$.
The single fiber response is {\color{red} (Manu, please help me on define this)}, with $D_{\parallel}=$ 2.2\e{-3}
  and $D_{\bot}=$ 0.2\e{-3} [mm s$^{-1}$].
...
\Gls*{csf} was simulated with a unique compartment with isotropic diffusivity $D_{CSF}$ of 3.0\e{-3} [mm s$^{-1}$].
Conversely, \Gls*{wm} and \gls*{gm} included one isotropic compartment with $D_{WM} =$ 2.0\e{-4}, $D_{cGM} =$ 7.0\e{-4}
  and $D_{dGM} =$ 9.0\e{-4}, respectively [mm s$^{-1}$].
...
\emph{Diffantom} is a digital phantom generated from a dataset belonging to the \acrlong*{hcp}, and the software layer to
  simulate it.
A first \emph{diffantom} is presented here to be openly and freely distributed along with the necessary software to reproduce
  this dataset and generate new \emph{diffantoms}.
...
Let $\{T'_{1\,\cdots\,5}\}$ be the set of original fractions maps obtained with the combined use of FAST and FIRST, FA the
  ...
  maps $V_i$ of single-fibers with their estimated volume fractions $F'_i$ calculated with BEDPOSTX.
The final $\{T_{1\,\cdots\,5}\}$ maps of isotropic fractions are computed as follows:
  ...
  T_5 &= 0.0
  \end{align*}
where $w_{\{cgm, dgm, wm\}}$ are weighing factors of anisotropic diffusion for each tissue
...
\end{align*}
where $w_{\{f1, f2, f3\}}$ are the contributions of the \gls*{gm} compartments to each single-fiber population.
...
{\textbf{Generation of \emph{diffantoms}:} First, a micro-structural model is generated as described
...
This micro-structural model is feeded into a simulation block that generates the final \gls*{dmri} dataset
...
\textbf{B1. Tractogram of fibers crossing slice 56 of the \emph{diffantom}} represented over that corresponding
  slice of the \emph{b0} volume. \textbf{B2. Segmentation of some fiber bundles}: in blue color is represented
  the right \gls*{cst}, in red the left \gls*{cst}, and in green the forceps minor.
  The box includes the slice 56 of the \emph{b0} and the light gray shadow is the pial surface.
}
\end{figure}
